...
public class TemperatureCurve implements TemperatureCurceInterface {
    ...
    public Temperature getMaxTemperatureByStream() {
        return temperatures.stream()
                .max(Comparator.comparing(Temperature::getTemperatureCelcius))
                .orElseThrow(() -> new NoSuchElementException("No max temperature found in the measurement list"));
    }


    public Temperature getMinTemperatureByStream() {
        return temperatures.stream()
                .min(Comparator.comparing(Temperature::getTemperatureCelcius))
                .orElseThrow(() -> new NoSuchElementException("No max temperature found in the measurement list"));
    }

    public List<String> getTemperatureListAsString() {
        return temperatures.stream().map(temperature -> Double.toString(temperature.getTemperatureCelcius())).collect(Collectors.toList());
    }

    public Collection<Temperature> getTemperaturesBetween20And60() {
        return temperatures.stream().filter(temperature -> (temperature.getTemperatureCelcius() >= 20 && temperature.getTemperatureCelcius() <= 60)).collect(Collectors.toList());
    }
